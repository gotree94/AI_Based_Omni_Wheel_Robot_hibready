...
* 'cv2_imencode()' 함수를 호출하여 'display' 출력 변수에 대이터를 저장한다. 데이터 저장형식은 'jpeg'와 객체감지 추론결과인 'result_image'를 넘겨준다.
* 단 'result_image'는 배열의 데이터를 가지고 있으므로 문자열의 형식은 'tostring()'함수를 이용해 문자열로 변환하여 배열의 '[1]' 데이터부터 넘겨준다.
